...
abstract class modexperienceHelper
{
    public static function getItems(&$params)
    {
        ...
        if( $items ):
            ...
            foreach( $items as $key => &$item ):
                $item->link = JRoute::_("index.php?option=com_experience&view=item&id={$item->id}&alias={$item->alias}&catid={$item->catid}") ;
            endforeach;
            ...
            foreach( $items as $key => &$item ):
                ...
                $item = new JObject();
                $item->a_title   = 'experience data - ' . ( $key +1 );
                $item->link      = '#' ;
