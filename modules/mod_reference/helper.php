...
abstract class modreferenceHelper
{
    public static function getItems(&$params)
    {
        ...
        if( $items ):
            ...
            foreach( $items as $key => &$item ):
                $item->link = JRoute::_("index.php?option=com_reference&view=item&id={$item->id}&alias={$item->alias}&catid={$item->catid}") ;
            endforeach;
            ...
            foreach( $items as $key => &$item ):
                ...
                $item = new JObject();
                $item->a_title   = 'reference data - ' . ( $key +1 );
                $item->link      = '#' ;
