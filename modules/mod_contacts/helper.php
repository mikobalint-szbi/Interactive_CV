...
abstract class modcontactsHelper
{
    public static function getItems(&$params)
    {
        ...
        if( $items ):
            ...
            foreach( $items as $key => &$item ):
                $item->link = JRoute::_("index.php?option=com_contacts&view=item&id={$item->id}&alias={$item->alias}&catid={$item->catid}") ;
            endforeach;
            ...
            foreach( $items as $key => &$item ):
                ...
                $item = new JObject();
                $item->a_title   = 'contacts data - ' . ( $key +1 );
                $item->link      = '#' ;
